...
async function postCrearTicket(req: NextApiRequest, res: NextApiResponse){
    ...
    try {
        const result = await new Connection().Query(`INSERT INTO TICKET(id_cliente_reportado,categoria_problematica) VALUES (${id_cliente_reportado}, ${categoria_problematica})`);
