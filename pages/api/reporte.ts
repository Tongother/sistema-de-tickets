...
async function postCrearTicket(req: NextApiRequest, res: NextApiResponse){
    const {id_cliente_reportado, categoria_problematica, fecha_reporte,fecha_cierre} = req.body;
    try {
        const result = await new Connection().Query(`INSERT INTO TICKET(id_cliente_reportado,categoria_problematica,fecha_reporte,fecha_cierre) VALUES (${id_cliente_reportado}, ${categoria_problematica}, ${fecha_reporte}, ${fecha_cierre})`);
